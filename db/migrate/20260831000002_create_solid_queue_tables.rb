# Solid Queue's schema (from solid_queue 1.7's install template), created
# in the primary database rather than a separate `queue` database -- this
# app has a single Postgres and no need to split it. Transcribed into a
# normal migration so `db:migrate` / `db:schema:load` manage it like every
# other table.
