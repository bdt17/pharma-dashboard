...
class PharmaTransportApp
  def self.call(env)
    case env["PATH_INFO"]
    when "/"
      [200, {"Content-Type" => "text/html"}, [File.read("public/index.html") || enterprise_login_html]]
    ...
    when "/billing", "/subscribe", "/landing", "/signup"
      [200, {"Content-Type" => "text/html"}, [stub_html(env["PATH_INFO"])]]
    else
    ...
    <<~HTML
      <div style="max-width:400px;margin:4rem auto;padding:2rem;border-radius:12px;box-shadow:0 10px 30px rgba(0,0,0,0.2);background:white">
        ...
        <form action="/users/sign_in" method="POST">
          <input type="email" name="user[email]" placeholder="admin@pharmatransport.com" required style="width:100%;padding:12px;border:1px solid #d1d5db;border-radius:8px;font-size:16px;box-sizing:border-box">
          <input type="password" name="user[password]" placeholder="pharma123" required style="width:100%;padding:12px;border:1px solid #d1d5db;border-radius:8px;font-size:16px;box-sizing:border-box;margin-top:1rem">
          <button type="submit" style="width:100%;background:#10b981;color:white;padding:12px;border:none;border-radius:8px;font-weight:600;font-size:16px;cursor:pointer;margin-top:1rem">LOGIN → Dashboard</button>
        </form>
        <p style="text-align:center;margin-top:1.5rem;color:#6b7280;font-size:14px">Demo: admin@pharmatransport.com / pharma123</p>
      </div>
    HTML
  end
      ...
      <div style="padding:2rem;background:#f8fafc min-height:100vh">
        <div style="max-width:1200px;margin:0 auto">
          ...
          <div style="background:#10b981;color:white;padding:1.5rem;border-radius:12px;margin-bottom:2rem">
            <strong>Phase 10 LIVE</strong> | 22/22 Endpoints ✅ | $5M ARR Ready
          </div>
          <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:1.5rem">
            <div style="background:white;padding:1.5rem;border-radius:12px;box-shadow:0 4px 6px rgba(0,0,0,0.05)">
              ...
              <p>Queclink GV55 → <strong>Phase 11 LIVE</strong></p>
            </div>
            <div style="background:white;padding:1.5rem;border-radius:12px;box-shadow:0 4px 6px rgba(0,0,0,0.05)">
