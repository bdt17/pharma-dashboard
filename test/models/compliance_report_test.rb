...
class ComplianceReportTest < ActiveSupport::TestCase
  ...
  test "requires a version and content_hash" do
  ...
  test "rejects a content_hash that isn't a real SHA-256 hex digest" do
    report = ComplianceReport.new(batch: @batch, organization: @organization, generated_by: @user, version: 1, content_hash: "not-a-hash")
    assert_not report.valid?
  ...
  test "rejects a duplicate version for the same batch" do
    ComplianceReport.create!(batch: @batch, organization: @organization, generated_by: @user, version: 1, content_hash: sha256("v1"))
    dupe = ComplianceReport.new(batch: @batch, organization: @organization, generated_by: @user, version: 1, content_hash: sha256("v1-again"))
  ...
  test "the same version number is fine on a different batch" do
    ...
    ComplianceReport.create!(batch: @batch, organization: @organization, generated_by: @user, version: 1, content_hash: sha256("a"))

    other = ComplianceReport.new(batch: other_batch, organization: @organization, generated_by: @user, version: 1, content_hash: sha256("b"))
    assert other.valid?
  end

  test "is immutable -- cannot be updated after creation" do
    report = ComplianceReport.create!(batch: @batch, organization: @organization, generated_by: @user, version: 1, content_hash: sha256("a"))
  ...
  test ".create_next_version! issues version 1 with no previous_hash for a batch's first report" do
    report = ComplianceReport.create_next_version!(batch: @batch, generated_by: @user, content_hash: sha256("first"))
  ...
  test ".create_next_version! chains each new version to the previous one's hash" do
    first = ComplianceReport.create_next_version!(batch: @batch, generated_by: @user, content_hash: sha256("v1"))
    second = ComplianceReport.create_next_version!(batch: @batch, generated_by: @user, content_hash: sha256("v2"))
